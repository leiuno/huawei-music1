  ...
  _createClass(Player, [{
    ...
    value: function start() {
      ...
      fetch('https://jirengu.github.io/data-mock/huawei-music/music-list.json').then(function (res) {
        ...
        console.log(data);
        _this.songList = data; //this.audio.src = this.songList[this.currentIndex].url
    ...
    key: "bind",
    value: function bind() {
      var self = this; //外面的this赋值给self，里面的this代表这个按钮
      //获取播放按钮节点，并绑定事件
    ...
    value: function playPreSong() {
      ...
      this.audio.src = this.songList[this.currentIndex].url; //this.audio.oncanplaythrough = () => this.audio.play()
    ...
    value: function loadlyrics() {
      ...
      fetch(this.songList[this.currentIndex].lyric).then(function (res) {
        ...
        _this3.setLyrics(data.lrc.lyric);
    ...
    value: function setLyrics(lyrics) {
      this.lyricsIndex = 0;
      var fragment = document.createDocumentFragment(); //创建文档片段，将元素附加到文档片段，然后将文档片段附加到DOM树
      //定义一个新数组，传入二维数组，0是时间，1是歌词

      var newArr = [];
      this.newArr = newArr; //字符串转换为数组，换行切割

      var lyricsArr = lyrics.split(/\n/);

      for (var i = 0; i < lyricsArr.length; i++) {
        var t = lyricsArr[i].match(/\[\d{2}:\d{2}((\.|\:)\d{2})\]/g);
        var str = lyricsArr[i].replace(/\[\d{2}:\d{2}((\.|\:)\d{2})\]/g, "");

        if (t != null) {
          for (var j = 0; j < t.length; j++) {
            var t_s = t[j];
            var min = Number(String(t_s.match(/\[\d{2}/i)).slice(1));
            var sec = parseFloat(String(t_s.match(/\d{2}\.\d{2}/i))); //换算时间

            var _t = Math.round((min * 60 + sec) * 1000); //把时间和对应的歌词保存到数组
      ...
      console.log(newArr);
      this.newArr.forEach(function (value) {
...
window.p = new Player('#player'); //循环播放，顺序播放，自动执行下一曲，检查若循环，下一曲还是index，
//音乐列表
